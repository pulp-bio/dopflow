...
class MyGUI:
    def __init__(self, root):
        ...
        self.port_combobox = ttk.Combobox(
            com_frame, textvariable=self.port_var, values=port_options, state="readonly"
        )
        ...
        graph_frame.grid(
            row=1, column=0, padx=10, pady=10, sticky=tk.W + tk.E + tk.N + tk.S
        )

        # Select Manual or pulse
        self.speed_mode_var = tk.StringVar(value="Manual")
        mode_radio1 = ttk.Radiobutton(
            graph_frame,
            text="Set manual speed",
        ...
        mode_radio2 = ttk.Radiobutton(
            graph_frame,
            text="Send blood flow pulses",
            variable=self.speed_mode_var,
            value="BFP",
            command=self.update_all,
        )
        ...
        self.load_file_button = ttk.Button(
            graph_frame, text="Load File", command=self.load_pulse_file
        )
        ...
        slider_frame.grid(
            row=2, column=0, rowspan=2, columnspan=4, padx=5, pady=5, sticky=tk.W
        )
        ...
        self.slider_amp = ttk.Scale(
            slider_frame,
            from_=0,
            to=self.MAX_SPEED,
            variable=self.slider_amp_var,
            orient=tk.HORIZONTAL,
            command=self.update_slider_amp,
        )
        self.slider_amp.grid(row=0, column=1, columnspan=2, padx=5, pady=5, sticky=tk.W)

        slider_amp_entry = ttk.Entry(
            slider_frame, width=5, textvariable=self.slider_amp_var
        )
        ...
        slider_freq = ttk.Scale(
            slider_frame,
            from_=10,
            to=120,
            variable=self.slider_freq_var,
            orient=tk.HORIZONTAL,
            command=self.update_slider_freq,
        )
        slider_freq.grid(row=1, column=1, columnspan=2, padx=5, pady=5, sticky=tk.W)

        slider_freq_entry = ttk.Entry(
            slider_frame, width=5, textvariable=self.slider_freq_var
        )
        ...
        self.send_button = ttk.Button(
            graph_frame, text="Start ▶️", command=self.send_to_com_button_call
        )
        ...
        self.custom_pulse = PulseWave(
            self.pulse.time_array, self.pulse.value_array, bpm, amp
        )
        ...
        self.figure, self.ax = (
            Figure(figsize=(5, 2), dpi=100, layout="tight", facecolor="#E4E5E4"),
            None,
        )
        ...
        self.serial_data_text = tk.Text(
            serial_frame, height=7, width=50, state="disabled"
        )
        ...
        self.com_check_thread = threading.Thread(target=self.com_check)
        self.com_check_thread.daemon = True
        self.com_check_thread.start()
    ...
    def send_speed(self, speed, unit: str = "m/s"):
        ...
        message = str(int(mot_speed + 0.5)) + "\n"
        try:
            self.ser.write(bytes(message, "utf-8"))
        except Exception as _:  # so that it doesn't crash if disconnected
            return 0
        else:
            return 1
    ...
    def plot_function(self):
        ...
        try:
            ...
            if self.unit_var.get() == "m/s":
                self.ax.set_ylim((-0.04, self.MAX_SPEED * 1.1))
            else:
                self.ax.set_ylim((-0.04, self.MAX_SPEED / self.ms_rpm_ratio * 1.1))
            self.canvas.draw()
    ...
    def load_pulse_file(self):
        self.send_speed(0)
        filetypes = (("Numpy array files", "*.npz"), ("All files", "*.*"))
        ...
        if filename != "":
            ...
            TIME_NAME = "time"
            VALUE_NAME = "value"
            if TIME_NAME not in file_data.keys() and VALUE_NAME not in file_data.keys():
                self.update_serial_data("Not a valid file format\n")
            else:
                self.custom_pulse.load_pulse(
                    file_data[TIME_NAME], file_data[VALUE_NAME]
                )
